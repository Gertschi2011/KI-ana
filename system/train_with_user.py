...
app = FastAPI(title="KI_ana Co-Learning API")
...
@app.post("/teach")
async def teach(payload: Dict[str, Any]):
...
async def rate(payload: Dict[str, Any]):
    if _mem is None:
        raise HTTPException(500, "memory_store not available")
...
@app.get("/colearn/gaps")
async def gaps(topic: Optional[str] = None):
    t = (topic or "").strip()
    if not t:
    ...
    if _reflect and hasattr(_reflect, "reflect_blocks_by_topic"):
        # gather blocks from disk via memory_store meta
        blocks = []
        try:
